...
TEST(BencodingTest, DecodesStrings) {
    ...
    auto res3 = bencoding::decode(input);
...
TEST(BencodingTest, DecodesLists) {
    // l 4:spam i42e e
    auto res = bencoding::decode("l2:hii42ee");

    // Check if it holds the correct type first (optional, but good practice)
    ASSERT_TRUE(std::holds_alternative<std::vector<bencoding::Bitem>>(res.val));
...
TEST(BencodingTest, DecodesDictionaries) {
    ...
    auto res = bencoding::decode("d3:bar4:spam3:fooi42ee");
...
TEST(BencodingTest, DecodesNestedComplex) {
    ...
    auto res = bencoding::decode(mock);
